#!/usr/bin/env bash
#
# Builds the three Comp76Fx plugins and installs the CLAP and VST3 of each
# into the user's plugin folders.
#
...
# All three revisions are separate plugins and install together.
readonly PLUGINS=("Comp76Fx Rev A" "Comp76Fx Rev D" "Comp76Fx Rev F")
readonly PACKAGES=(comp76fx_rev_a comp76fx_rev_d comp76fx_rev_f)
...
if [ "$build" = true ]; then
    ...
    (cd "$project_dir" && cargo xtask bundle "${package_args[@]}" --release)
fi
...
for plugin in "${PLUGINS[@]}"; do
    install_bundle "$plugin.clap" "$clap_dir"
    install_bundle "$plugin.vst3" "$vst3_dir"
done
