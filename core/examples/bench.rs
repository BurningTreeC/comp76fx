...
use comp76fx_core::dsp::{revisions, Channel, Controls, Detector, Revision};
use comp76fx_core::params::dial_position;

const FS: f64 = 192_000.0;

/// The shipped revisions with their noise switched off, so the figures read
/// the circuit rather than the floor.
const REV_A: Revision = revisions::REV_A.without_noise();
const REV_D: Revision = revisions::REV_D.without_noise();
const REV_F: Revision = revisions::REV_F.without_noise();
...
fn harmonics(rev: Revision, c: Controls, input_db: f64) -> (f64, f64) {
    ...
    let f1 = bin(1.0);
    (
        20.0 * (bin(2.0) / f1).log10(),
        20.0 * (bin(3.0) / f1).log10(),
    )
}
...
fn main() {
    if std::env::args().any(|a| a == "--spec") {
        ...
        for hz in [
            20.0, 30.0, 50.0, 100.0, 1000.0, 10_000.0, 16_000.0, 20_000.0,
        ] {
        ...
        check(
            ...
            format!("{worst:+.2} dB at {worst_hz:.0} Hz"),
            worst.abs() <= 1.0,
        );
        ...
        for (label, knob, want, fastest) in [
            ...
            let _ = fastest;
            let set =
                detector::knob_to_time(knob, detector::ATTACK_FASTEST, detector::ATTACK_SLOWEST);
            let (a, _) = detector_times(knob, 0.5);
            check(
                &format!("{label} ({:.0} us)", want * 1e6),
                format!("{:.1} us", a * 1e6),
                (a - set).abs() / set < 0.05 && (set - want).abs() / want < 1e-6,
            );
        }
        for (label, knob, want) in [
            ...
            let set =
                detector::knob_to_time(knob, detector::RELEASE_FASTEST, detector::RELEASE_SLOWEST);
            let (_, r) = detector_times(0.5, knob);
            check(
                &format!("{label} ({:.0} ms)", want * 1e3),
                format!("{:.1} ms", r * 1e3),
                (r - set).abs() / set < 0.05 && (set - want).abs() / want < 1e-6,
            );
        }
        ...
        for rev in [REV_A, REV_D, REV_F] {
            ...
            check(
                &format!("distortion, {} at -18 dBFS, under 0.5 %", rev.name),
                format!("{t:.3} %"),
                t < 0.5,
            );
        }
        ...
        check(
            "gain reduction available, at least 40 dB",
            format!("{gr:.1} dB"),
            gr >= 40.0,
        );
        ...
        for (i, marked) in [4.0, 8.0, 12.0, 20.0].iter().enumerate() {
            ...
            println!(
                "  marked {marked:>5.0}:1   linearised {got:8.4}:1   error {:+.3} %",
    ...
    if std::env::args().any(|a| a == "--combos") {
        ...
        sets.sort_by_key(|b| {
            (
                ...
                b.iter().position(|x| *x).unwrap(),
            )
        });
        for b in sets {
            let label: Vec<&str> = names
                .iter()
                .zip(b)
                .filter(|(_, p)| *p)
                .map(|(n, _)| *n)
    ...
    if std::env::args().any(|a| a == "--vs20") {
        ...
        for lvl in [-36.0, -30.0, -24.0, -18.0, -12.0] {
            ...
            println!(
                "  {lvl:>5.0} dBFS   {:>7.2} dB {:>8.2} dB   {:>8.3} % {:>8.3} %",
                ca.gain_reduction_db(),
                ct.gain_reduction_db(),
                thd_rev(REV_D, a, lvl),
                thd_rev(REV_D, t, lvl)
            );
        }
    ...
    if std::env::args().any(|a| a == "--presets") {
        ...
        for name in [
            ...
            let Some(dials) = comp76fx_core::presets::built_in_dials(name) else {
                continue;
            };
            let dial = |id: &str| dials.iter().find(|(n, _)| *n == id).map(|(_, v)| *v as f64);
            let (Some(input), Some(output)) = (dial("input"), dial("output")) else {
                continue;
            };
            let c = Controls {
                input_db: input,
                ...
                buttons: [
                    ...
                    dial("ratio12").unwrap_or(0.0) > 0.5,
                    dial("ratio20").unwrap_or(0.0) > 0.5,
                ],
            };
            let mut ch = Channel::new(REV_D, FS, 4, 1);
            ch.set_controls(c);
            ...
            println!(
                "  {name:<22} {input:>5.0} {output:>7.0} {gr:>7.2} {:>7.2} dB {thd:>7.2} %",
                out + 18.0
            );
        }
        return;
    ...
    if std::env::args().any(|a| a == "--allbutton") {
        ...
        for lvl in [-30.0, -24.0, -18.0, -12.0] {
            ...
            println!(
                "  {lvl:>5.0} dBFS   {:>7.2} dB {:>8.2} dB   {:>8.3} % {:>8.3} %",
                lvl + 17.0 - 11.0 - a_out - 11.0 + 11.0,
                lvl + 17.0 - f_out - 11.0 + 11.0 - 17.0 + 17.0,
                thd_rev(REV_D, preset, lvl),
                thd_rev(REV_D, four, lvl)
            );
        }
        ...
        for lvl in [-30.0, -24.0, -18.0, -12.0] {
            ...
            println!(
                "  {lvl:>5.0} dBFS   all-in {all_gr:>6.2} dB    4:1 {:>6.2} dB",
                ch.gain_reduction_db()
            );
        }
        ...
        for lvl in [-48.0, -44.0, -40.0, -36.0, -32.0, -28.0, -24.0] {
            let mut a = Channel::new(REV_D, FS, 4, 1);
            a.set_controls(preset);
            let mut f = Channel::new(REV_D, FS, 4, 1);
            f.set_controls(four);
            let amp = 10f64.powf(lvl / 20.0);
            let w = std::f64::consts::TAU * 1000.0 / FS;
            for n in 0..(FS as usize * 2) {
    ...
    if std::env::args().any(|a| a == "--times") {
        ...
        for (label, k) in [
            ...
            let want =
                detector::knob_to_time(k, detector::ATTACK_FASTEST, detector::ATTACK_SLOWEST);
            let (a, _) = detector_times(k, 0.5);
            println!(
                "  {label}  set {:>7.1} us -> measured {:>8.1} us  ({:.2}x)",
                want * 1e6,
                a * 1e6,
                a / want
            );
        ...
        for (label, k) in [
            ...
            let want =
                detector::knob_to_time(k, detector::RELEASE_FASTEST, detector::RELEASE_SLOWEST);
            let (_, r) = detector_times(0.5, k);
            println!(
                "  {label}  set {:>7.1} ms -> measured {:>8.1} ms  ({:.2}x)",
                want * 1e3,
                r * 1e3,
                r / want
            );
    ...
    for drive in [0.0, 5.0, 10.0, 15.0, 20.0, 25.0, 30.0, 35.0, 40.0] {
        ...
        println!(
            "  input {drive:>4.0} dB -> out {out:7.2} dBFS   GR {:5.2} dB",
            -30.0 + drive - out
        );
    }
